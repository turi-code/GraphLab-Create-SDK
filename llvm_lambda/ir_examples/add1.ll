...
define i32 @"__main__.add_one$1.int64"(i64* nocapture %retptr, { i8*, i32 }** nocapture readnone %excinfo, i8* nocapture readnone %env, i64 %arg.x) #0 {
entry:
  %.13 = add nsw i64 %arg.x, 1
  store i64 %.13, i64* %retptr, align 8
  ret i32 0
}

define i8* @"wrapper.__main__.add_one$1.int64"(i8* %py_closure, i8* %py_args, i8* nocapture readnone %py_kws) {
entry:
  %.5 = alloca i8*, align 8
  %.6 = call i32 (i8*, i8*, i64, i64, ...)* @PyArg_UnpackTuple(i8* %py_args, i8* getelementptr inbounds ([8 x i8]* @.const.add_one, i64 0, i64 0), i64 1, i64 1, i8** %.5)
  %.7 = icmp eq i32 %.6, 0
...
entry.endif.endif:                                ; preds = %entry.endif, %entry.endif.if
  %.12.0 = phi i64 [ %phitmp, %entry.endif.if ], [ undef, %entry.endif ]
...
entry.endif.endif.endif.endif.endif:              ; preds = %entry.endif.endif.endif.endif
  ...
  %.68 = call i8* @PyInt_FromLong(i64 %.12.0)
  br label %entry.if
}
